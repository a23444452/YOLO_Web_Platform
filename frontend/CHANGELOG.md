...
**使用建議**:
- 對於高品質模型，使用預設值（0.7 / 0.8）可獲得更精準的結果
- 如果偵測數量不足，可在自動標註對話框中手動調低參數
- 推薦範圍：信心度 0.5-0.9，IOU 0.5-0.9

**修改檔案** (`src/pages/Annotation.tsx`):
- 第 50 行：`useState([0.25])` → `useState([0.7])`
- 第 51 行：`useState([0.45])` → `useState([0.8])`
